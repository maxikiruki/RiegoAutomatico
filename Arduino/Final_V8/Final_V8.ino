...
String time_init = "";
String time_final = "";
int hora_inicial = NULL;
int minuto_inicial = NULL;
...
void loop() {
  ...
  if (regado &&  !regar) {
    ...
    int dia = now.day() ;
    //fecha = anyo + "-" + mes + "-" + dia ;
    int hora_final = now.hour() ;
    int minuto_final = now.minute() ;
    ...
    char insert_sector1[] = "INSERT INTO riegoautomatico.history (id,sector_id, date,start_time,end_time,start_humidity,final_humidity,total_liters) VALUES  (NULL,'1','%02d-%02d-%d','%02d:%02d','%02d:%02d','%d','%d','%d');";
    Serial.println("368");
    char query[200];
    Serial.println("370");
    sprintf(query, insert_sector1, anyo, mes, dia, hora_inicial, minuto_inicial, hora_final, minuto_final, Humedad_init, Humedad_final, TotalLitros);
    Serial.println("372");
